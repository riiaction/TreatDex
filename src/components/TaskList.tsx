...
export function TaskList({
  ...
  useEffect(() => {
    ...
    // Only consider setting a toast if there's actual positive progress.
    // Progress calculation might momentarily drop if a subset subtask is toggled off. We only trigger toasts on forward momentum.
    // However, simplest logic: we only care about hitting the threshold.
  ...
  const sortedTasks = [...tasks].sort((a, b) => {
    ...
    return 0; // maintain relative order for tie or no deadlines
  });
    ...
    <div classname="flex-1 relative flex flex-col overflow-hidden">
      <div classname="flex-1 overflow-y-auto pb-32">
        <div classname="px-6 pt-4 pb-6">
          {/* Apple Reminders Style Summary Grid */}
          ...
          {/* Tags Panel */}
          {showTagsPanel && (
            <motion.div initial="{{" height:="" 0,="" opacity:="" 0,="" scale:="" 0.95="" }}="" animate="{{" height:="" "auto",="" opacity:="" 1,="" scale:="" 1="" }}="" classname="mb-6 p-4 pt-5 bg-white rounded-2xl shadow-sm border border-gray-100 flex flex-wrap gap-2 relative origin-top">
               <button onclick="{()" ==""> {
                   setShowTagsPanel(false);
                   setTaskFilter('all');
                   setSelectedTag(null);
          ...
          <div classname="space-y-4 mb-4 mt-2">
            {filteredTasks.length === 0 ? (
              ...
              filteredTasks.map((task) => (
                <taskitem key="{task.id}" task="{task}" reward="{rewards.find((r)" ==""> r.id === task.rewardId)}
        ...
        {/* Progress Bar absolute at bottom of relative container */}
        <div classname="absolute bottom-6 left-0 right-0 px-6 max-w-2xl mx-auto pointer-events-none z-10 flex flex-col items-center">
          <animatepresence>
            {toastMessage && (
              <motion.div initial="{{" opacity:="" 0,="" y:="" 10="" }}="" animate="{{" opacity:="" 1,="" y:="" 0="" }}="" exit="{{" opacity:="" 0,="" y:="" -10="" }}="" transition="{{" duration:="" 0.3="" }}="" classname="mb-2 bg-yellow-400 text-yellow-900 px-5 py-2.5 rounded-full text-sm font-medium shadow-md whitespace-nowrap text-center">
                {toastMessage}
              </motion.div>
            )}
          </AnimatePresence>
...
const TaskItem: React.FC<{
  ...
  return (
    <div classname="relative rounded-2xl mb-6">
      ...
      <motion.div drag="x" dragconstraints="{{" left:="" 0,="" right:="" 0="" }}="" dragelastic="{{" left:="" 0.5,="" right:="" 0="" }}="" dragdirectionlock="" ondragend="{handleDragEnd}" animate="{controls}" whiletap="{{" cursor:="" "grabbing"="" }}="" classname="{cn(" "relative="" rounded-2xl="" p-4="" shadow-sm="" border="" transition-colors="" cursor-pointer="" w-full="" h-full="" z-10",="" task.completed="" ?="" "border-[#1daeb1]="" 30="" bg-[#e6f7f7]"="" :="" "bg-white="" border-gray-100="" hover:border-[#1daeb1]="" 30",="" )}="" ondoubleclick="{(e)" ==""> {
          e.stopPropagation();
          if (onEditTask) onEditTask();
        }}
      >
        <div classname="flex items-start gap-4">
          {/* Checkbox for main task if no subtasks, or if it has subtasks it's decorative? Let's make it functional anyway */}
          ...
          <div classname="flex-1">
            <div classname="flex flex-wrap items-center gap-2">
              <h3 classname="{cn(" "text-lg="" font-bold="" transition-all",="" task.completed="" ?="" "line-through="" text-gray-500"="" :="" "text-gray-800",="" )}="">
                {task.title}
              </h3>
              ...
              <div classname="mt-3 space-y-2">
                {task.subtasks.map((st) => (
                  <div key="{st.id}" classname="flex items-center gap-2 group">
                    ...
                    <span classname="{cn(" "text-sm",="" st.completed="" ?="" "text-gray-400="" line-through"="" :="" "text-gray-600",="" )}="">
                      {st.title}
                    </span>
                  </div>
                ))}
          ...
          >
            <div classname="{cn(" "w-14="" h-14="" rounded-full="" flex="" items-center="" justify-center="" transition-all="" shadow-sm="" flex-shrink-0="" overflow-hidden="" relative",="" task.completed="" ?="" "bg-[#fdf3d0]="" border-2="" border-[#facc15]="" p-[10%]"="" :="" "bg-gray-100="" border-2="" border-gray-100",="" )}="">
              <img src="{" task.completed="" ?="" getpokemonsprite(reward="" ?="" reward.pokemonid="" :="" task.pokemonid)="" :="" getpokeballsprite()="" }="" alt="Reward" classname="{cn(" "object-contain="" transition-transform",="" task.completed="" ?="" "w-full="" h-full="" drop-shadow-sm"="" :="" "w-7="" h-7="" opacity-50="" grayscale="" m-auto",="" )}="" style="{{" imagerendering:="" "pixelated"="" }}=""/>
            </div>
          </div>
        </div>
      </motion.div>
