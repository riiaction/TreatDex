...
export function MasterKeyModal({
  ...
  const [activeMasterTab, setActiveMasterTab] = useState<"pokedex" | "rewards">("pokedex");
            ...
            <div className="flex-1 overflow-y-hidden flex flex-col mt-4">
              <div className="flex justify-between items-center mb-6 flex-shrink-0">
                <div className="flex gap-2 p-1 bg-gray-100 rounded-xl">
                  <button
                    onClick={() => setActiveMasterTab("pokedex")}
                  ...
                  <div className="overflow-y-auto pr-2 flex-1 relative">
                    <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3 pb-10">
                      {KANTO_POKEMON.map((name, index) => {
                        ...
                        const isAssigned = rewards.some((r) => r.pokemonId === pokemonId);

                        return (
                          <button
                            key={pokemonId}
                            onClick={() => openRewardEditor(pokemonId, name)}
                          ...
                          >
                            <img
                              ...
                              alt={name}
                              className="w-12 h-12 object-contain scale-[1.3]"
                ...
                <div className="flex-1 overflow-y-auto flex flex-col min-h-0">
                        ...
                        <div className="flex-1 space-y-2.5">
                          <div className="flex gap-2">
                            {reward.type === "cafe" ? (
                              <PlacesInput
                                value={reward.name || ""}
                                onChange={(e) => onUpdateReward(reward.id, { name: e.target.value })}
                            ...
                            <select
                              ...
                              onChange={(e) => {
                                ...
                                if (newType === "link" && reward.type === "cafe") {
                                  updates.name = "";
                                  updates.url = "";
                                  updates.content = "";
                                }
                            ...
                            <button onClick={() => onDeleteReward(reward.id)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors flex-shrink-0">
                              <Trash2 size={18} />
                            </button>
                          </div>
                          ...
                          {reward.type === "cafe" && (
                            <div className="flex items-center gap-2 bg-white border border-gray-200 rounded-lg px-3 py-2 text-sm focus-within:border-[#1daeb1] focus-within:ring-1 focus-within:ring-[#1daeb1] transition-colors">
                              ...
                              <input
                                ...
                                onChange={(e) => onUpdateReward(reward.id, { content: e.target.value })}
                          ...
                          {(reward.type === "cafe" || reward.type === "link") && (
                            <div className="flex items-center gap-2 bg-white border border-gray-200 rounded-lg px-3 py-2 text-sm focus-within:border-[#1daeb1] focus-within:ring-1 focus-within:ring-[#1daeb1] transition-colors">
                              <LinkIcon size={14} className="text-gray-400 flex-shrink-0" />
                              <input
                                type="text"
                                value={reward.url || ""}
                                onChange={(e) => onUpdateReward(reward.id, { url: e.target.value })}
                                onBlur={async (e) => {
                              ...
                              {reward.url && (
                                <a
                                  href={reward.url.startsWith("http") ? reward.url : `https://${reward.url}`}
                                  target="_blank"
                                  rel="noopener noreferrer"
                                  className="text-gray-400 hover:text-[#1daeb1] transition-colors flex-shrink-0"
                                >
                  ...
                  <button
                    onClick={() => {
                      ...
                      const availablePokes = allIds.filter((id) => !usedPokemonIds.includes(id));
                      const nextPoke = availablePokes.length > 0
              ...
              <div className="mt-6 pt-6 border-t border-gray-200 flex-shrink-0">
                <button onClick={() => setShowUpdateModal(true)} className="w-full bg-gray-100 hover:bg-gray-200 text-gray-800 font-bold py-3 rounded-xl text-sm transition-colors flex items-center justify-center gap-2">
      ...
      {selectedPokemon && editingReward && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-3xl w-full max-w-sm p-6 shadow-2xl relative">
            <button onClick={() => setSelectedPokemon(null)} className="absolute top-4 right-4 p-2 bg-gray-100 rounded-full text-gray-500 hover:bg-gray-200">
              <X size={16} />
            </button>
            ...
            <div className="space-y-4">
              <div>
                <label className="text-xs font-bold text-gray-500 uppercase ml-1 mb-1 block">Reward Name</label>
                <div className="flex gap-2">
              ...
              <div>
                <label className="text-xs font-bold text-gray-500 uppercase ml-1 mb-1 block">Reward Type</label>
                <select
                  value={editingReward.type || "cafe"}
                  onChange={(e) => {
                    const newType = e.target.value as any;
                    const updates: Partial<Reward> = { type: newType };
              ...
              {editingReward.type === "cafe" && (
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      type="text"
                      value={editingReward.content || ""}
                      onChange={(e) => setEditingReward({ ...editingReward, content: e.target.value })}
              ...
              {(editingReward.type === "cafe" || editingReward.type === "link") && (
                <div>
                  <label className="text-xs font-bold text-gray-500 uppercase ml-1 mb-1 block">Link</label>
                  <div className="relative">
                    <LinkIcon size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
                    <input
                      type="url"
                      value={editingReward.url || ""}
                      onChange={(e) => setEditingReward({ ...editingReward, url: e.target.value })}
                      onBlur={async (e) => {
            ...
            <div className="flex gap-3 mt-8">
              <button onClick={() => setSelectedPokemon(null)} className="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold py-3 rounded-xl transition-colors">
                Cancel
              </button>
            ...
            <form onSubmit={handleUpdateMasterKey} className="space-y-4">
              <div className="relative">
                <input
                  ...
                  required
                />
                <button type="button" onClick={() => setShowOldKey(!showOldKey)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
              ...
              <div className="relative">
                <input
                  ...
                  required
                />
                <button type="button" onClick={() => setShowNewKey(!showNewKey)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
              ...
              <div className="relative">
                <input
                  ...
                  required
                />
                <button type="button" onClick={() => setShowConfirmKey(!showConfirmKey)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
